...
def main():
    ...
    oFile = open("us_flow_13_14_inflow.txt","wb")
    mTable13 = {}
    mTable14 = {}
    for mRow in mFile13:
    ...
    for ele in mTable14.keys():
        #print ele
        if ele in mTable13:
            if int(mTable13[ele]) != int(mTable14[ele]):
                mString = mTable14[ele] + ',' + mTable13[ele] + '\n'
                oFile.write(mString)
    oFile.close()
    mFile14.close()
    mFile13.close()
...
def flow_count():
    print "count starts"
    mFile = open("us_flow_13_14_inflow.txt","rb")
    oFile = open("us_flow_13_14_inflow_countsxxxx.txt","wb")
    moves = []
    for mRow in mFile:
