...
def correlate(pathIn1,pathIn2,pathOut):
	...
	for nRow in mFileIRS:
		...
		mTableIRS[(fields[-2],fields[-1])] = fields[2]
	for ele in mTableTwitter.keys():
		if ele in mTableIRS:
			mString = mTableTwitter[ele] + ',' + mTableIRS[ele] + '\n'
			oFile.write(mString)
		else:
			mString = mTableTwitter[ele] + ',' + '0' + '\n'
			oFile.write(mString)
	for ele in mTableIRS.keys():
		if ele in mTableTwitter:
			print(ele)
		else:
			mString = '0' + ',' + mTableIRS[ele] + '\n'
			oFile.write(mString)
	oFile.close()
	mFileTwitter.close()
	mFileIRS.close()
...
def main():
	...
	correlate("us_flow_14_15_outflow_counts.txt","migration_outflow_IRS_1415_FID.csv","twitter_IRS_1415_zero.csv")
