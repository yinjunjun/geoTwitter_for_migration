...
def flow_coords(path1, path2):
	mFile = open(path1,"r")
	mCoord = open("county_fip_coord.csv","r")
	oFile = open(path2, "w")
	table = {}
	for mRow in mCoord:
		data = mRow.rstrip().split(',')
		table[data[-1]] = [data[2], data[1]]
	for nRow in mFile:
		fields = nRow.rstrip().split(',')
		if fields[0] in table and fields[1] in table:
			mString = ','.join(table[fields[0]]) + ',' + ','.join(table[fields[1]]) + ',' + nRow
			oFile.write(mString)
	oFile.close()
	mCoord.close()
	mFile.close()
...
def main():
	...
	# correlate_pairs("us_flow_14_15_outflow_counts.txt","migration_outflow_IRS_1415_FID.csv","twitter_IRS_outflow_pairs_1415.csv")	
	...
	# flow_coords("twitter_IRS_outflow_pairs_1314.csv","twitter_IRS_outflow_pairs_coords_1314.csv")
	# flow_coords("twitter_IRS_inflow_pairs_1314.csv","twitter_IRS_inflow_pairs_coords_1314.csv")
